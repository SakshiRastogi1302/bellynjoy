    ...
    return(
        <div className='flex flex-wrap space-x-10 p-2'>
            {foodList.map((food_item) => {
                return ( 
                    <div key={food_item.id} className='w-64 h-96 p-2 m-5 space-y-3 rounded-lg border-2 border-[#e72929]'>
                        <img src={`/${food_item.imageUrl}`} alt={food_item.name} className='w-64 rounded-md'></img>
